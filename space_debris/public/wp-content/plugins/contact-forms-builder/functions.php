 ...
 *
 * front-end and back-end scripts and style-sheets are also loaded
...
//	Enable widget supports for WpDevArt contact forms
if ( ! defined( 'ABSPATH' ) ) exit;

add_filter('widget_text', 'do_shortcode');
add_action('init', 'wpda_form_initial_set' );
//

/*############  CSS function ################*/

function wpda_form_initial_set() {
	wp_register_style('wpda_form_gutenberg_css',wpda_form_PLUGIN_URI.'wpdevart-inc/gutenberg/style.css');
	wp_register_script('wpda_form_gutenberg_js',wpda_form_PLUGIN_URI.'wpdevart-inc/gutenberg/block.js',array( 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'underscore' ));
}

// Add front-end scripts - function
...
//	Add contact form shortcode anywhere to display form at front-end.
...
//	Function that add submission shortcode anywhere to display form submissions at front-end
 ...
 * 	if field's subfields found return founded subfields,
...
//	Function that search if styling exists for a form and return if it has
...
//	Function that check if styling exists for any form
...
function wpda_form_forms_custom_styles() {
	...
	if (!empty($get_styles)) 
	{
		foreach($get_styles as $key => $style)  {
			//	If styling disabled don't add styling to front-end 
...
//	Form size line height function 
...
//	Contact form font style function
...
//	This function used in making csv export file of form submissions
...
//	This function check similiar form name and append integer to it  e.g form, form, form-1, form-2 
...
//	This function checks if current author (e.g username='john_doe') is allowed to view content
...
//	delete directory after scanning the given directory and removes the files in it if any.
